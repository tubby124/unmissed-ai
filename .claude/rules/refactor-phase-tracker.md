...
| D82 | **GAP — call stages testing** | D81 (call stages) is code-complete but untested live. Steps: (1) run `/prompt-deploy hasan-sharif` to push updated prompt with BOOKING CHECK line, (2) make a test call — say name + "I want to book a showing", (3) confirm Aisha calls `transitionToBookingStage` with callerPhone/callerName/serviceType, (4) confirm booking stage fires, calendar checked, appointment booked. Check DB first: `booking_enabled=true` + `calendar_auth_status='connected'` on hasan-sharif. | HIGH | NOT STARTED |
