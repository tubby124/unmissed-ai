...
Step 3 — If caller already named a specific time AND that time appears in the slots: skip listing — go straight to confirming that exact slot. Only read back up to 3 options when the caller has NOT named a time, or when their requested time is unavailable.
