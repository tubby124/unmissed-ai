/**
 * D445 Hasan-Sharif Snowflake Migration — Dry-Run Phase 2
 *
 ...
 *
 * Outputs:
 *   - hasan-snowflake-pre-migration.json (current DB state)
 *   - hasan-snowflake-dryrun.json (recomposePrompt result with preview + currentPrompt)
 */
...
// Inline service client — avoids importing next/headers from src/lib/supabase/server.ts
const svc = createClient(SUPABASE_URL, SERVICE_KEY, { auth: { persistSession: false } })

const OUT_DIR = path.resolve('CALLINGAGENTS/00-Inbox')

async function main() {
  console.log('[1/4] Looking up hasan-sharif client row...')
  const { data: client, error: clientErr } = await svc
    .from('clients')
    .select(
      'id, slug, niche, business_name, agent_name, agent_voice_id, ' +
  ...
  if (clientErr || !client) {
    throw new Error(`hasan-sharif lookup failed: ${clientErr?.message ?? 'not found'}`)
  }
  ...
  console.log('[2/4] Resolving an admin user_id (for prompt_versions audit row)...')
  ...
  console.log('[3/4] Writing pre-migration snapshot...')
  const preMigrationPath = path.join(OUT_DIR, 'hasan-snowflake-pre-migration.json')
  fs.writeFileSync(preMigrationPath, JSON.stringify(client, null, 2))
  console.log(`  wrote ${preMigrationPath}`)

  console.log('[4/4] Running recomposePrompt(dryRun=true, forceRecompose=true)...')
  ...
  const dryrunPath = path.join(OUT_DIR, 'hasan-snowflake-dryrun.json')
