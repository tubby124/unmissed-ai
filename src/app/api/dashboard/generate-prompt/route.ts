...
export async function POST(req: NextRequest) {
  ...
  const { data: existingClient } = await svc
    .from('clients')
    .select('id, ultravox_agent_id, forwarding_number, booking_enabled, sms_enabled, knowledge_backend, transfer_conditions, listing_search_url')
    .eq('slug', clientSlug)
    .maybeSingle()
  ...
  try {
    ...
    if (existingAgentId) {
      ...
      const agentFlags: Parameters<typeof updateAgent>[1] = {
        ...
        listing_search_url: (existingClient?.listing_search_url as string | null) || undefined,
      }
  ...
  await insertPromptVersion(svc, {
    clientId,
    content: prompt,
    changeDescription: `Auto-generated from intake (niche: ${niche}, ${validation.charCount} chars)`,
