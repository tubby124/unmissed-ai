# 21st.dev Voice Components — Saved 2026-03-23

## Component 1: AI Voice Input (ai-voice-input.tsx)

Simple voice input with mic button, timer, and audio visualizer bars.
...
## Component 2: Voice Powered Orb (voice-powered-orb.tsx)

WebGL shader orb (OGL library) that reacts to voice/microphone input.
Dependencies: ogl (NEW — needs `npm install ogl`)
    ...
    try {
      ...
      return () => {
        ...
        if (container && glContext && glContext.canvas) {
          try { if (container.contains(glContext.canvas)) { container.removeChild(glContext.canvas); } } catch (error) { console.warn("Canvas cleanup error:", error); }
        }
        stopMicrophone();
...
- `ogl` — WebGL library for VoicePoweredOrb (NOT currently installed)
- `lucide-react` — already installed for AIVoiceInput

## Notes

- VoicePoweredOrb uses WebGL shaders via OGL — heavier than CSS but way more visually impressive
- AIVoiceInput is pure CSS/DOM — lightweight, simulated visualizer (not real audio data)
- VoicePoweredOrb actually connects to the microphone and analyzes real audio data
- Both components are SSR-safe ("use client" + isClient checks)
- VoicePoweredOrb `hue` prop allows color customization (0-360 degrees)
- Integration candidate: replace the CSS orb in `AgentTestCard` / `LiveTestCall` with VoicePoweredOrb during active calls
