 ...
 *
 * Ultravox HMAC format (verified 2026-03-22, docs.ultravox.ai/webhooks/securing-webhooks):
 ...
 *   CRITICAL: Use secrets[0] from POST /api/webhooks response as the HMAC key.
 *   Do NOT use a self-provided secret — Ultravox transforms it internally.
 */
...
export async function POST(req: NextRequest) {
  ...
  if (!signatureValid) {
    console.error(`[ultravox-webhook] HMAC signature mismatch. ts=${timestamp} bodyLen=${rawBody.length}`)
