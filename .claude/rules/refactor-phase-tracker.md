...
| D81 | **FEATURE — agent mode** | Phase 5 (Call Stages / booking stage transition). For `lead_capture` and `appointment_booking` clients with `booking_enabled=true`, transition mid-call to a focused booking stage via Pattern D. (1) `src/app/api/stages/[slug]/booking/route.ts` returns `X-Ultravox-Response-Type: new-stage` with focused booking prompt + calendar tools only. (2) `buildBookingTransitionTool(slug)` in `ultravox.ts` added to `buildCalendarTools()`. (3) Prompt line injected into `triageDeep` in `prompt-builder.ts` when `booking_enabled=true`. Build verified. **NOT YET LIVE-TESTED.** | MEDIUM | **DONE** 2026-03-29 (code complete, build ✅ — needs live test call to verify stage transition fires) |
| D82 | **GAP — call stages testing** | D81 (call stages) is code-complete but untested live. Need to: (1) confirm a client has `booking_enabled=true` + connected calendar, (2) run `/prompt-deploy [slug]` to push updated prompt with `transitionToBookingStage` line, (3) make a test call, say name + service during triage, confirm agent calls `transitionToBookingStage`, (4) confirm booking stage fires and calendar availability is checked. Hasan-sharif or windshield-hub are candidates — check `booking_enabled` + `calendar_auth_status` in DB first. | HIGH | NOT STARTED |
| D83 | **GAP — stages route not protected by plan gate** | `POST /api/stages/[slug]/booking` route checks `booking_enabled` but NOT plan entitlement (`plan.bookingEnabled`). A Lite/Core client with `booking_enabled=true` could in theory hit this route. Low risk since `buildCalendarTools()` gates the transition tool on `plan.bookingEnabled` anyway (tool never registers for non-Pro), but the route itself should match the same gate for consistency. Fix: fetch `selected_plan` from DB in the stage route and check `getPlanEntitlements(plan).bookingEnabled`. | LOW | NOT STARTED |
...
DONE  -> S0-S9.6, S12 Ph1, S13, S13.5, S18 partial, S19a,
         ...
         D81 (call stages booking transition — code complete, build ✅, live test pending — 2026-03-29)
