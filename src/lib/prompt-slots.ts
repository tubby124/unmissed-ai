...
export function buildForbiddenActions(ctx: SlotContext): string {
  ...
  const kbPriming = !kbAvailable
    ? ''
    : ctx.kbStance === 'strict'
      ? `\n\nDEFAULT for ANY factual question (fees, policies, procedures, pet rules, application steps, lease terms, warranty, what's included, how something works): say a bridge phrase ('yeah let me check that one for you... one sec') FIRST, BEFORE any substantive content, THEN call queryKnowledge. If the tool returns an approved answer, share it naturally. If it returns nothing, route to ${ctx.closePerson}: "I don't have that exact one — ${ctx.closePerson} can confirm when they call you back." NEVER invent specifics before the tool fires.\n\nEXCEPTION — IDENTITY questions only (these FIVE topics): 1) where you serve / areas / cities you cover, 2) your hours, 3) your business model / what kind of business you are, 4) high-level "what do you do," 5) who the owner is. For these five only, answer DIRECTLY from the Identity section in your context. NO bridge phrase. NO queryKnowledge. If the question doesn't clearly match one of the FIVE, treat as DEFAULT.\n\nDo NOT call queryKnowledge for greetings, emergencies, or booking confirmations.`
      : `\n\nDEFAULT for ANY factual question (fees, policies, procedures, pricing, lease terms, what's included), call queryKnowledge first. Say a bridge phrase BEFORE the tool fires. If the tool returns an approved answer, share it naturally. Only route to ${ctx.closePerson} when queryKnowledge returns no results.\n\nEXCEPTION — IDENTITY questions only (FIVE topics: where you serve, your hours, your business model, what you do, owner name): answer DIRECTLY from the Identity section. NO bridge. NO queryKnowledge.\n\nDo NOT call queryKnowledge for greetings, emergencies, or booking confirmations.`
...
2. Never say "certainly," "absolutely," "of course," or "I will." Use the acknowledgment rotation in TONE. Avoid "gotcha" / "mhm" — too slangy for business calls.
3. ${pricingRule}
...
Respond the moment they finish speaking. Let them interrupt — stop gracefully when they do. Rotate acknowledgments — pull from "got it", "sure", "okay", "yeah", "perfect", "no problem", "no worries", "sounds good". Use any one at most twice per call, never on consecutive turns. Avoid "gotcha" / "mhm" — too casual for business calls.`
