...
export function buildForbiddenActions(ctx: SlotContext): string {
  ...
  const kbPriming = !kbAvailable
    ? ''
    : ctx.kbStance === 'strict'
      ? `\n\nIDENTITY questions (where you serve, your hours, your business model, what you do, owner name) — answer DIRECTLY from the Identity section in your context. Do NOT bridge. Do NOT call queryKnowledge.\n\nFor POLICY questions (specific fees, application steps, lease terms, pet rules, warranty details, procedures), call queryKnowledge first. If the tool returns an approved answer, share it naturally. ONLY route to ${ctx.closePerson} when (a) queryKnowledge returns no results, OR (b) the question is about a specific case / unit / tenant situation / property / patient / file (not a general policy). Do NOT call queryKnowledge for greetings, emergencies, or booking confirmations.`
      : `\n\nIDENTITY questions (where you serve, your hours, your business model, what you do, owner name) — answer DIRECTLY from the Identity section in your context. Do NOT bridge. Do NOT call queryKnowledge.\n\nFor POLICY questions (specific fees, lease terms, pricing details, procedures), call queryKnowledge first. If the tool returns an approved answer, share it naturally. Only route to ${ctx.closePerson} when queryKnowledge returns no results. Do NOT call queryKnowledge for greetings, emergencies, or booking confirmations.`
