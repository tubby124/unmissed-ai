...
6. **Failed-transfer recovery is smart, not amnesiac.** When the owner doesn't answer and the call reconnects to the AI, [transfer-status/route.ts:220-245](src/app/api/webhook/[slug]/transfer-status/route.ts#L220-L245) injects a `TRANSFER FAILED` note into caller context AND sets `firstSpeakerText` to "Hey, looks like they're tied up right now. Would you like to leave a message and I'll make sure they get it?" The AI opens with that line, not a fresh "Thanks for calling" greeting. Customer never hears a confused restart. Earlier claim that the reconnected AI "has no memory" was wrong — it has explicit failure context + a hardcoded recovery opener.
