...
| `business_name` | DB_PLUS_PROMPT | `clients.business_name` | `PATCH /api/dashboard/settings` | YES — `patchBusinessName()` replaces old name with new name throughout `system_prompt` (word-boundary matched) | None | YES — triggers `needsAgentSync` when `system_prompt` changes | No | No | Dashboard header, Telegram alerts, Settings → Agent Identity card | Low — patcher is word-boundary matched (same as `patchAgentName`) | Implemented via `settings-patchers.ts` | `patchBusinessName()` in `lib/prompt-patcher.ts` |
...
### Risk 6: `business_name` not re-patched after provision — RESOLVED (D282)
**Status:** FIXED (2026-03-31). `settings-patchers.ts` now handles `business_name` via `patchBusinessName()` — word-boundary replace throughout `system_prompt`. Mutation class updated to `DB_PLUS_PROMPT`. Change triggers `computeNeedsSync` → Ultravox agent resynced automatically.
