...
function IntakeRow({ intake, onCreateAccount, onGeneratePrompt, onActivate }: {
  ...
  const nicheConfig = getNicheConfig(intake.niche)
        ...
        <td className="px-4 py-3">
          <div className="flex items-center gap-3">
            ...
            <div className="min-w-0">
              <p className="text-sm font-medium truncate" style={{ color: "var(--color-text-1)" }}>{intake.business_name}</p>
              <div className="flex items-center gap-1.5 mt-0.5">
                {nicheConfig ? (
                  ...
                  <span className="text-xs capitalize" style={{ color: "var(--color-text-3)" }}>{intake.niche?.replace(/_/g, ' ') || '—'}</span>
                )}
              </div>
            </div>
          </div>
          ...
          <>
            ...
            {missingClientId && (
              <p className="text-xs rounded px-3 py-2 mb-2" style={{ background: 'var(--color-warn-bg, #fef9c3)', color: 'var(--color-warn-text, #854d0e)' }}>
                Generate prompt first (Actions &rarr; Generate Prompt) before activating.
              </p>
            )}
...
export default function IntakeTable({ intakes }: { intakes: Intake[] }) {
