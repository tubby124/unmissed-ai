...
NEXT:
  D89  -> Fix scheduled-callback disposition write-back (1-line check in completed/route.ts)
  D96  -> Fix XML entity escaping in VM script TwiML (outbound-connect/route.ts)
  D91  -> Fix AMD URL length bomb (token table approach)
  D90  -> Fix Ultravox billing leak on machine-detected calls
