...
export async function POST(req: NextRequest) {
  ...
  const { data: clientRow, error: clientErr } = await supa
    ...
    .insert({
      ...
      // Wave 3 Layer C — dual-mode awareness. Defaults to true (matches DB default)
      // so the universal PERSONAL flow trunk fires for solo owners forwarding
      // their personal cell. Owner can toggle via the onboarding Call Routing
      // step or the dashboard Call Routing card post-onboard.
      is_forwarding_personal_cell: intakePayload.is_forwarding_personal_cell ?? true,
      carrier_id: intakePayload.carrier_id ?? null,
    })
    .select("id")
    .single();
